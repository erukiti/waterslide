...
var BinGenerator = function () {
    function BinGenerator(operator) {
        _classCallCheck(this, BinGenerator);

        this.operator = operator;
    }

    _createClass(BinGenerator, [{
        key: 'generate',
        value: function () {
            var _ref = _asyncToGenerator(regeneratorRuntime.mark(function _callee(name) {
                var opts = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
                var binText, jsInstaller;
                return regeneratorRuntime.wrap(function _callee$(_context) {
                    while (1) {
                        switch (_context.prev = _context.next) {
                            case 0:
                                binText = fs.readFileSync(path.join(__dirname, 'sample.bin.js'));

                                this.operator.writeFile('bin/' + name, binText, { mode: 493 });
